...
To evaluate the impact of genetic variation on gene expression, we performed eQTL mapping, doing so at three distinct scopes (see \textbf{Methods}): the local region of a gene (defined as within 10Mb of the gene transcription start site (TSS); Analysis L), the chromosome (Analysis C), and the genome (Analysis G). Additionally, we classify distal-eQTL located on the same chromosome as the associated gene as intra-chromosomal, and otherwise inter-chromosomal.
...
We also identified eQTL with Analyses C and G using a more relaxed significance threshold (FDR $<$ 0.2; \nameref{S_grid_plot_lenient}A and \nameref{S_eqtl_lenient}), which increased the number of distal-eQTL detected by a greater extent than local-eQTL. Using Analysis L and requiring genome-wide significance, the percentage of tested genes with local-eQTL were 8.4\% for liver, 6.3\% for lung, and 9.5\% for kidney (\nameref{S_eqtl}). These percentages increase to 19.8\%, 16.6\%, and 20.8\% respectively with the less stringent Analysis L at chromosome-wide significance. Most genes with local-eQTL were observed in only one tissue (\nameref{S_upset_qtl}A).
