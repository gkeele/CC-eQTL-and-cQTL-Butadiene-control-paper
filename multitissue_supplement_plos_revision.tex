...
\end{equation}
where ``A $\indep$ B'' indicates that A and B are independent of each other and ``A $|$ B'' is A conditioned on B, then X acts on Y completely through M, which is commonly referred to as full mediation. Partial mediatory relationships are also possible in which X affects Y both directly and through M, traditionally identified by testing for $M \quad \rightarrow \quad Y \quad | \quad X$. In practice, the true relationships among the variables can be complex and obscured by noise, making it hard to completely distinguish full mediation, partial mediation, and X separately affecting M and Y. Furthermore, genome-wide data present a significant challenge to systematically testing these relationships because of the presence of many candidate mediators, potential correlation structure due to linkage disequilibrium (LD), and error heterogeneity across the levels of genomic data. For these reasons, we do not use the traditional mediation tests \cite{Baron1986}, but adapt and simplify them for genomic data.

\subsection*{Scans}

We perform mediation scans, described in Eq 3 of the Methods. These scans test for significant reduction in the relationship $X \quad \rightarrow \quad Y \quad | \quad M$, akin to the full mediator relationship (relationship 5), though without attempting to define a formal threshold for independence. The relationship is tested for all mediators within the local region of the eQTL ($\pm$10Mb).
