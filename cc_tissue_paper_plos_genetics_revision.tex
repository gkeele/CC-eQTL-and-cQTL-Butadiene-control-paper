...
\subsection*{\GKinline{Fewer detected cQTL than eQTL}}

\WVinline{We find, similar to other studies\WV{}{add refs}, smaller effect sizes on average and fewer detections for cQTL than eQTL (see \nameref{S_qtl_effect_sizes_by_method}). This is likely for reasons both technical and biological. Technically, the RNA-seq assay measures a distinct class of molecules (mRNAs) that can be accurately extracted from cells, and the resulting sequence reads are then mapped to the transcriptome, which encompasses $<$ 5\% of the genome. In contrast, the transposon incorporation event central to ATAC-seq is enriched in, but not solely limited to, accessible chromatin. Accessible chromatin, unlike the transcriptome, can occur anywhere in the genome and regions are defined empirically by the data. The signal from the chromatin assay is therefore more variable and noisy across samples, which impedes our ability to detect cQTL. Biologically, if a variant affects the activity of a regulatory element that alters expression levels, this will be invariably detected as increased (or decreased) mRNAs in the sample. By contrast, if a variant affects chromatin accessibility, it may do so in a manner that is difficult to detect above the background noise of the assay. Two recent studies of adult brain in the same study population found 2,154,331 \textit{cis}-eQTL using $n=467$ individuals \cite{Richard2012} but only 6,200 cQTL for $n=272$ individuals \cite{Bryois2018}, a reduction in detection rate far greater than would be expected due to sample size alone.}

\subsection*{\GKinline{Reduced cQTL and mediation signal in liver compared with lung or kidney}}

\GKinline{Detecting QTL and mediators depends on not only sample size but also biological and technical factors that are difficult to quantify across the tissues. Potential differences in technical factors limit the ability to make strong biological conclusions based on differences in numbers of QTL or mediators, or the lack thereof, in any given tissue relative to the other tissues. Although it is possible that liver has fewer actual cQTL, and thus fewer chromatin mediators, than lung and kidney, it is also possible that the ATAC-seq data was noisier. If there are true biological differences in the number of cQTL and in mediator usage among the tissues, they likely reflect tissue-specific multi-level regulatory programs, a complex subject requiring more targeted experiments than used here.}
