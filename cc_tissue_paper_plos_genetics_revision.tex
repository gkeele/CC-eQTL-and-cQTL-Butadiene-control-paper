...
\TFinline{MPPs such as the CC and DO are increasingly being generated in diverse species, including \textit{Arabidopsis}, \textit{Drosophila}, and yeast, to aid in the investigation of genetic associations with molecular and phenotypic traits \cite{DeKoning2017}.}

\GKinline{Similar to studies of humans, these} mouse resources have \GKinline{only recently} been employed in mediation analyses across genomic assays. A genome-wide mediation approach in 192 DO mice was used to link transcriptional and post-translational regulation of protein levels \cite{Chick2016}; the CC were used to confirm results by showing that estimates of founder haplotype effects from each of the related populations corresponded. More recently, mediation analysis was used to connect chromatin accessibility with gene expression in embryonic stem cells derived from DO mice \cite{Skelly2019}. 
\GKinline{The CC and DO, and MPPs more broadly, are powerful tools for mediation analyses because their well-characterized genetic diversity (\ie founder LD blocks) provides more informative patterns by which to relate the various levels of data, in comparison to bi-allelic SNP variants.}
...
\GKinline{The use of only male mice was for practical reasons, and we note could vary in female mice \cite{Chick2016}.} As the function of each tissue is quite distinct, we expected that these data would reflect tissue-specific differences. Principal components analysis (PCA) of each of the gene expression and chromatin accessibility profiles showed that the samples clearly clustered by tissue (\nameref{S_pca}). 
...
%% A 
(A) Detected QTL are largely local for both gene expression and chromatin accessibility (A). QTL detected through all analyses, excluding intra-chromosomal distal-QTL detected through Analysis C, are included. The y-axis represents the genomic position of the gene or chromatin site, and the x-axis represents the genomic position of the QTL. Local-QTL appear as dots along the diagonal and distal-QTL as off-diagonal dots. The gray dashed box highlights lung and kidney QTL results, which are further explored, including comparisons of pairs shared between both tissues.
%% B
Highly significant QTL map nearby the gene TSS and chromatin window midpoint (B). Results from Analysis G are shown here. The red dashed line represents $\pm$ 10Mb from the trait genomic coordinate used to classify a QTL as local or distal. See \nameref{S_dist} for all the tissues as well as results from Analysis C.
%% C
Local QTL have larger effect sizes than distal (C). Only results from Analysis G are included here. See \nameref{S_effect_size_status} for all tissues and results from Analysis C.
%% D
Consistent genetic regulation of gene expression and chromatin accessibility was observed between lung and kidney tissue, based on an enrichment in positively correlated founder haplotype effects for QTL paired across the two tissues (D). QTL from all the Analyses were considered here. See \nameref{S_qtl_pair_histograms} for all tissue pairings.
%% E
QTL observed in both lung and kidney that are highly correlated tend to map close to each other, consistent with representing genetic variation active in both tissues (E). See \nameref{S_qtl_cor_by_distance_comparison} for all tissue pairings.
%% F
The effect sizes of QTL paired from lung and kidney are lowly but significantly correlated (F). See \nameref{S_qtl_effect_size_comparison} for the tissue comparisons.}}
...
\GKinline{For local-eQTL, we found significant haplotype effect correlations between 345 of 761 possible pairs (45.3\%) in liver/lung, 623 of 1206 (51.7\%) in liver/kidney, and 498 of 1025 (48.6\%) in lung/kidney. For distal-eQTL, we found significant correlations between 21 of 61 possible pairs (31.8\%) in liver/lung, 34 of 120 (28.8\%) in liver/kidney, and 16 of 59 (27.1 \%) in lung/kidney. For cQTL, the vast majority of correlated pairs were local, with 47 of 55 (85.5\%) possible pairs in liver/lung, 48 of 56 (85.7\%) in liver/kidney, and 118 of 142 (83.1\%) in lung/kidney being significantly correlated. Only 4 distal-cQTL pairs were observed, all between lung and kidney. As such, the correlations were not formally tested; however, three of the four pairs had correlations greater than 0.5. The effect sizes of paired QTL varied across tissues, though for any given tissue pairing, the effect sizes were significantly correlated (least significant $p$-value =\num{4.6e-8}; Fig \ref{fig:qtl_results}F and \nameref{S_qtl_effect_size_comparison}). No significantly negatively correlated QTL pairs were detected, after accounting for multiple testing (FDR $\le 0.1$), consistent with being tissue-specific QTL with transgressive effects when compared across tissues.} \GK{}{I am trying to say that the few anti-correlated pairs we do see likely represent transgressive QTL active in separate tissues.}
...
We primarily report fixed effects estimates due to their consistency with reported expectations \cite{Keele2019} for a study of this size. %We believe the conservative shrinkage-based estimates were overly reduced. 
These QTL mapping results are largely consistent with \GKinline{the molecular traits with detected QTL possessing primarily Mendelian genetic regulation (large effect sizes: $>$ 60\%). Though the relatively limited number of CC strains ($<$ 70 strains) greatly constrains their effectiveness in mapping highly complex and polygenic traits, this study supports their value for simpler but high throughput traits, particularly when considering further gains from replicate observations per strain and joint analysis with the DO and the founder strains.}
Shrunken effect size estimates can be used to identify putative QTL with unstable fixed effect estimates that result from extreme data points, and thus aid in the identification of low confidence QTL. Notably, a small number of the distal-eQTL had low random effect size estimates (\nameref{S_qtl_effect_size_fixefvsranef}) compared to their fixed effects-based estimates. Alternatively, a residual variation estimate, \ie RSS, could be calculated from the shrunken haplotype effects to identify likely false positives, but not be as aggressively reduced as the variance component-based estimates, representing a middle ground approach that was found to be effective \cite{Keele2018}.
