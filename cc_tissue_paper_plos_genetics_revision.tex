...
\WVinline{For a given trait, QTL from different tissues were paired if they co-localized to approximately the same genomic region. In the case of local-QTL, both had to be within the local window, defined as 10Mb around the gene TSS or chromatin region midpoint, such the maximum distance between them would be 20Mb. In the case of distal-eQTL, they had to be within 10Mb.
Consistency between these cross-tissue QTL pairs was assessed by calculating the correlation between their founder haplotype effects (FDR $\le 0.1$; Fig \ref{fig:qtl_results}D and \nameref{S_qtl_pair_histograms}), with significant positive correlations implying that the paired QTL acted similary and were multi-tissue QTL.}
