...
Consider an eQTL that our previous genome scan has already shown to affect the expression of gene $j$. Denote the expression level for $j$ in individual $i$ as  $y^{\text{G}_{j}}_{i}$ and eQTL effect as $\text{eQTL}_{i}^{\text{G}_{j}}$ (these respectively correspond to $Y$ and $X$ in the mediation description above). Further, consider a potential mediator $k\in\mathcal{K}$, where $\mathcal{K}$ is the set of all \WVinline{eligible chromatin accessible sites or expressed genes}, and let $m_{ik}$ be the value of that mediator for individual $i$. The mediation scan for a given gene/eQTL pair $j$ proceeds by performing, for each potential mediator $k\in\mathcal{K}$, a model comparison between the alternative model,}
\begin{equation}
