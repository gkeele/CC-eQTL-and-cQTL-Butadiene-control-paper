...
\newcommand{\GK}[2]{\textcolor{teal}{#1\footnote{\textcolor{red}{GK: #2}}}}
...
%% A 
Detected QTL are largely local for both gene expression and chromatin accessibility (A). QTL detected through all analyses, excluding intra-chromosomal distal-QTL detected through Analysis C, are included. The y-axis represents the genomic position of the gene or chromatin site, and the x-axis represents the genomic position of the QTL. Local-QTL appear as dots along the diagonal and distal-QTL as off-diagonal dots. The gray dashed box highlights lung and kidney QTL results, which are further explored, including comparisons of pairs shared between both tissues.
%% B
...
For a given trait, QTL from different tissues were paired based on co-localizing to approximately the same genomic region. For local-QTL, both had to be within the local window, defined as 10Mb around the gene TSS \GKinline{or chromatin region midpoint}, resulting in a maximum distance of 20Mb between QTL. For distal-eQTL, both had to be within 10Mb of each other. \GKinline{To assess the consistency between the QTL paired across tissue, the correlations between the founder haplotype effects were tested (FDR $\le 0.1$; Fig \ref{fig:qtl_results}D and \nameref{S_qtl_pair_histograms}), revealing significant positive correlations as would be expected for QTL active in both tissues (multi-tissue QTL).}
\GKinline{For local-eQTL, we found 345 correlated of 761 possible pairs (45.3\%) in liver/lung, 623 of 1206 (51.7\%) in liver/kidney, and 498 of 1025 (48.6\%) in lung/kidney. For distal-eQTL, we found 21 of 61 possible pairs (31.8\%) in liver/lung, 34 of 120 (28.8\%) in liver/kidney, and 16 of 59 (27.1 \%) in lung/kidney. For cQTL, the vast majority of correlated pairs were local, with 47 of 55 (85.5\%) possible pairs in liver/lung, 48 of 56 (85.7\%) in liver/kidney, and 118 of 142 (83.1\%) in lung/kidney. Only 4 distal-cQTL pairs were observed, all between lung and kidney. As such, the correlations were not formally tested; however, three of the four pairs had correlations greater than 0.5. The effect sizes of paired QTL vary across tissues, though for any given tissue pairing, the effect sizes were significantly correlated (least significant $p$-value =\num{4.6e-8}; Fig \ref{fig:qtl_results}F and \nameref{S_qtl_effect_size_comparison}). No significantly negatively correlated QTL pairs were detected, after accounting for multiple testing (FDR $\le 0.1$), consistent with being tissue-specific QTL with transgressive effects when compared across tissues.}
