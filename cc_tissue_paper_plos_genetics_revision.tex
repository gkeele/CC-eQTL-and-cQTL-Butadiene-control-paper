...
\WV{Effect size estimates provide interpretable point summaries for haplotype-based QTL mapping, analogous to minor allele effect estimates in SNP-based studies. We calculated estimates of effect size in two ways, one based on a fixed effect fitting of the QTL term and the other as a random term \cite{Wei2016}. 
...
Consider an eQTL that our previous genome scan has already shown to affect the expression of gene $j$. Denote the expression level for $j$ in individual $i$ as  $y^{\text{G}_{j}}_{i}$ and eQTL effect as $\text{eQTL}_{i}^{\text{G}_{j}}$ (these respectively correspond to $Y$ and $X$ in the mediation description above). Further, consider a potential mediator $k\in\mathcal{K}$, where $\mathcal{K}$ is the set of all \WV{eligible chromatin accessible sites or expressed genes}{Greg: further qualifications?}, and let $m_{ik}$ be the value of that mediator for individual $i$. The mediation scan for a given gene/eQTL pair $j$ proceeds by performing, for each potential mediator $k\in\mathcal{K}$, a model comparison between the alternative model,}
\begin{equation}
...
\WV{This model comparison can be seen as re-examining the significance of a given eQTL association by conditioning on each mediator in turn (\ie, testing $X\rightarrow Y|M$ for each $M$);}
in comparison to the QTL genome scan in which the QTL effect is fit at positions across the genome, the mediation scan has the locus fixed at the eQTL, and the mediator term is fit for candidates across the genome.
Additional covariates including conditioned loci can be included in both the null and alternative models, but not shown here for simplicity. 

Because the eQTL is always included in the alternative model but not the null model, the logP of the mediation scan should fluctuate around the observed eQTL logP. At mediators that possess some or all of the information present in the eQTL, a reduced logP is observed compared with the eQTL logP\GKinline{, reflecting $X\rightarrow Y|M$ being less significant that  $X\rightarrow Y$}. 
...
See \nameref{S3_Appendix} for further discussion of mediation analysis, including the permutation approach and the formal criterion by which mediation is declared for both chromatin and gene mediators (Fig \ref{fig:graph}).
