...
For pathway analysis of differentially chromatin accessiblility \GKinline{near genes}, each region was mapped to a gene using GREAT v3.0.0 (\textit{basal plus extension} mode, \textit{5 kb upstream}, \textit{1 kb downstream}, and no distal extension). 
\WV{Each gene was then paired with the chromatin region with the smallest FDR-adjusted $p$-value.}{in the same location? I'm not understanding this.}  
...
\GKinline{The minimum DAR $q$-value was selected as the summary $q_{g}$ for chromatin near gene $g$.} Gene weights were calculated as with gene expression, but with $\text{sign}(\Delta_{g})$ representing the sign of chromatin accessibility fold-change and $q_{g}$ the FDR-adjusted $p$-value of the chromatin region paired with gene $g$.
...
\begin{equation}
  y_i = \mu + \text{batch}_{b[i]} + \text{QTL}_i + \varepsilon_i\, \label{eq:alternative_model}
\end{equation}
...
\subsection{Calling detected QTL: local (L), local chromosome (LC), and genomewide (G)}
...
These protocols used two types of multiple test correction: a permutation-based control of the familywise error rate (FWER) and the Benjamini-Hochberg False Discovery Rate (FDR) \cite{}. Below we describe the permutation procedure, and then the three protocols.

\subsubsection{Permutation-based significance thresholds: chromosome- and genome-wide.}

\WV{}{I reordered this subsubsection a bit but it needs a bit more cleaning up.} Thresholds were determined based on permutations specific to each trait. 1,000 permutations of the sample index were recorded, and then genome scans performed for each trait, using the same permutations. Given a trait, the maximum logP, from either the entire genome or the local chromosome, for each permutation was collected, and used to fit a null \GKinline{generalized extreme value distribution (GEV)} \citep{Dudbridge2004}. Genome-wide and local chromosome-wide thresholds were calculated as the 95\textsuperscript{th} upper quantile of the respective EVDs. \WV{}{And explain genomewide and chromosomewide adjustment. Use this section to define shorthands you can use in the descriptions of the analyses below.}
...
Our detection criteria for local-QTL leverages a strong prior belief that there will be local genetic regulation. For a given trait, this local analysis involved examining QTL associations at all loci within a 10Mb window of the gene transcription start site (TSS) or chromatin region midpoint. A QTL was detected if the logP of the maximum peak (the peak logP) in the window was above the chromosome-wide significance threshold. This peak logP is reported after chromosome-wide adjustment.
...
Local chromosome analysis involved examining QTL associations at all loci on the local chromosome, \ie, the chromosome harboring the gene or chromatin region. The peak logP is subject to chromosome-wide adjustment. \WV{The chromosome-wide adjusted logPs for all traits of a given type (expression or chromatin) are then further subject to adjustment by FDR.}{correct? may need additions}
...
\WV{
\GKinline{Genomewide analysis ... incorporates additional scans conditioned on detected QTL to potentially identify multiple QTL per trait in unbiased fashion for FDR. Notably, the local status of the QTL does not factor into Analysis G. See \textbf{Appendix D} for details on the conditional genome scan procedure.}}{Re-write this pgh and provide enough details to understand it, following the style of the descriptions above.}
