...
120 Mason Farm Rd., Genetic Medicine Bldg., Dept of Genetics, University of North Carolina, Chapel Hill, NC 27599.

E-mail: william.valdar@unc.edu, tsfurey@email.unc.edu}
\vspace{-11pt}%
...
We also identified eQTL with Analyses C and G using a more relaxed significance threshold (FDR < 0.2; Figure \ref{fig:grid_fdr_plot}A and Table \ref{tab:eqtl_mapping_lenient}), which increased the number of distal-eQTL detected by a greater extent than local-eQTL. Using Analysis L and requiring genome-wide significance, the percentage of tested genes with local-eQTL were 8.4\% for liver, 6.3\% for lung, and 9.5\% for kidney (Table \ref{tab:eqtl_mapping}). These percentages increase to 19.8\%, 16.6\%, and 20.8\% respectively with the less stringent Analysis L at chromosome-wide significance. Most genes with local-eQTL were observed in only one tissue (Figure \ref{fig:upset_eqtl_cqtl}A).
