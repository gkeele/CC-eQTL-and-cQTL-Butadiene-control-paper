...
\subsection{\GKinline{QTL effect summaries}}\GK{}{I have reorganized the following sections a bit. Previously we reported eQTL and cQTL separately, even though descriptions of effect size patterns became very redundant. To avoid that, I've tried to put all that in one section: QTL effect summaries.}

\GKinline{We estimated the effect size for each detected QTL. The effect size estimates should not be over-interpreted, as the Beavis effect, \ie QTL effect size inflation, on a sample of 47 strains is expected to be biased upward \citep{KeeleSPARCC}; however the estimates can be used to compare QTL detected by different methods.}
